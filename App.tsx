...
import { playStartSound, playSuccessSound, playErrorSound } from './utils/soundUtils';
...
const Button = ({ label, onClick, className = "", secondary = false, accent = false }: any) => (
  <button
    onClick={onClick}
    className={`
...
const App: React.FC = () => {
  ...
  const calculate = useCallback(() => {
    ...
    switch (operator) {
      ...
      case '-': result = previous - current; break;
      case '*': result = previous * current; break;
      case '/': result = current === 0 ? 0 : previous / current; break;
  ...
  const renderKeypad = () => {
    ...
    const basicKeys: KeyButton[] = [
      { l: 'AC', fn: clear }, { l: 'DEL', fn: del }, { l: '%', fn: () => handleOperator('/') }, { l: '÷', fn: () => handleOperator('/') },
      { l: '7', fn: () => handleNumber('7') }, { l: '8', fn: () => handleNumber('8') }, { l: '9', fn: () => handleNumber('9') }, { l: '×', fn: () => handleOperator('*') },
      { l: '4', fn: () => handleNumber('4') }, { l: '5', fn: () => handleNumber('5') }, { l: '6', fn: () => handleNumber('6') }, { l: '-', fn: () => handleOperator('-') },
      { l: '1', fn: () => handleNumber('1') }, { l: '2', fn: () => handleNumber('2') }, { l: '3', fn: () => handleNumber('3') }, { l: '+', fn: () => handleOperator('+') },
      { l: '0', fn: () => handleNumber('0'), w: 'col-span-2' }, { l: '.', fn: () => handleNumber('.') }, { l: '=', fn: calculate, accent: true },
    ];

    const scientificKeys: KeyButton[] = [
      { l: 'sin', fn: () => {}, secondary: true }, { l: 'cos', fn: () => {}, secondary: true }, { l: 'tan', fn: () => {}, secondary: true }, { l: '√', fn: () => handleOperator('√') },
      { l: 'log', fn: () => {}, secondary: true }, { l: 'ln', fn: () => {}, secondary: true }, { l: '(', fn: () => {}, secondary: true }, { l: ')', fn: () => {}, secondary: true },
      { l: '^', fn: () => handleOperator('^'), secondary: true }, { l: 'π', fn: () => handleNumber('3.1415'), secondary: true },
    ];

    const keysToRender = mode === AppMode.SCIENTIFIC ? [...scientificKeys, ...basicKeys] : basicKeys;
    const gridCols = mode === AppMode.SCIENTIFIC ? 'grid-cols-4' : 'grid-cols-4';

    return (
      <div className={`grid ${gridCols} gap-3 p-4 flex-1 content-end pb-24`}>
    ...
    <div className="flex flex-col h-screen bg-slate-900 text-slate-100 overflow-hidden font-sans">
      ...
      <div className="flex items-center justify-between p-4 bg-slate-900 z-10">
        <div className="flex gap-2 overflow-x-auto no-scrollbar">
          {[AppMode.NORMAL, AppMode.SCIENTIFIC, AppMode.FINANCE, AppMode.CONVERTER].map((m) => (
            <button
              key={m}
              onClick={() => setMode(m)}
              className={`px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap transition-colors ${
                mode === m ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-400'
              }`}
            >
              {m}
            </button>
        ...
        <button onClick={() => setMode(AppMode.HISTORY)} className="ml-2 p-2 text-slate-400 hover:text-emerald-400">
          <History size={20} />
        </button>
      </div>
      ...
      <div className="flex flex-col items-end justify-end p-6 space-y-2 bg-gradient-to-b from-slate-900 to-slate-800 flex-shrink-0 min-h-[180px] relative">
        ...
        {voiceFeedback && (
          ...
          `}>
            {voiceState === 'LISTENING' && <span className="inline-block mr-2"><Mic size={16} className="inline animate-bounce"/></span>}
            "{voiceFeedback}"
          </div>
        )}
      ...
      {mode === AppMode.FINANCE ? (
         <div className="flex-1 p-6 space-y-4">
             <div className="bg-slate-800 p-4 rounded-xl border border-slate-700">
               <h3 className="text-emerald-400 font-bold mb-4">Calculadora de Juros (Simples)</h3>
               <div className="space-y-3">
                 <input className="w-full bg-slate-900 p-3 rounded text-white" placeholder="Principal (P)" type="number" onChange={(e) => setPrevVal(e.target.value)} />
                 <input className="w-full bg-slate-900 p-3 rounded text-white" placeholder="Taxa % (r)" type="number" onChange={(e) => setOperator(e.target.value)} />
                 <input className="w-full bg-slate-900 p-3 rounded text-white" placeholder="Tempo (t)" type="number" onChange={(e) => {
        ...
        <div className="flex-1 p-6 space-y-4">
          <div className="bg-slate-800 p-4 rounded-xl border border-slate-700">
             <h3 className="text-blue-400 font-bold mb-4">Conversor (C° para F°)</h3>
             <div className="flex gap-2 items-center">
               <input 
                 ...
                 onChange={(e) => {
                    ...
                    if(!isNaN(c)) setInput(((c * 9/5) + 32).toFixed(1));
                 }} 
               />
