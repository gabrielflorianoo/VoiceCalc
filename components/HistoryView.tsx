...
import { ArrowLeft, Trash2, ShoppingBag, TrendingUp, TrendingDown, MapPin } from 'lucide-react';
...
const HistoryView: React.FC<HistoryViewProps> = ({ history, onBack, onClear }) => {
  ...
  const groupedData = useMemo(() => {
    ...
    return Object.values(groups).sort((a, b) => b.records[b.records.length-1].timestamp - a.records[a.records.length-1].timestamp);
  }, [history]);

  return (
    <div className="flex flex-col h-full bg-slate-900 text-slate-100 p-4 overflow-hidden">
      <div className="flex items-center justify-between mb-6">
        <button onClick={onBack} className="p-2 bg-slate-800 rounded-full text-slate-300">
          <ArrowLeft size={24} />
        </button>
        <h2 className="text-xl font-bold flex items-center gap-2">
          <ShoppingBag size={20} className="text-emerald-400" /> Histórico
        </h2>
        <button onClick={onClear} className="p-2 bg-rose-900/30 text-rose-400 rounded-full">
          <Trash2 size={20} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pb-24 space-y-4">
        {history.length === 0 ? (
          <div className="text-center text-slate-500 mt-20">
            ...
            <p className="text-sm">Use o comando de voz "Salvar Compra" após um cálculo.</p>
          </div>
        ) : (
          groupedData.map((group) => (
            <div key={group.location} className="bg-slate-800 rounded-xl p-4 shadow-lg border border-slate-700">
              <div className="flex justify-between items-start mb-2">
                <h3 className="font-bold text-lg text-white flex items-center gap-2">
                  <MapPin size={16} className="text-slate-400" /> {group.location}
                </h3>
              ...
              {group.records.length > 1 && (
                <div className="mt-3 pt-3 border-t border-slate-700">
                   <p className="text-xs text-slate-500 mb-1">Histórico recente neste local:</p>
                   <div className="space-y-1">
                     {group.records.slice().reverse().slice(0, 3).map(rec => (
                       <div key={rec.id} className="flex justify-between text-xs">
                         ...
                         <span className={rec.amount > group.lastAmount ? 'text-green-400' : 'text-slate-300'}>
